...
namespace Drupal\sous\Commands;
...
/**
 * Hooks into Drush to assist with uninstalling Sous.
 *
 * @internal
 *   This class is a completely internal part of Sous's uninstall system
...
final class Uninstaller extends DrushCommands implements SiteAliasManagerAwareInterface {
  ...
  /**
   * Defines dynamic options if uninstalling Sous.
   *
   ...
   *
   * @return string|null
   *   If an uninstall of Sous is being attempted,
  ...
  /**
   * Validates the pm:uninstall command if Sous is being uninstalled.
   *
   ...
   *   Thrown if:
   *   - The user is trying to uninstall Sous at
   *     the same time as other extension(s).
   *   - Any installed extensions are physically located inside the Sous
   *     profile directory.
   *   - Any profiles (installed or not) are using Sous as their immediate
   *     parent, and the user declines to modify them automatically.
   */
  public function validate(CommandData $data): void {
    ...
    if ($profile) {
      ...
      // Ensure that there are no installed modules or themes in the Sous
      // profile directory.
      $extensions = $this->getExtensionsInProfileDirectory();
      if ($extensions) {
        $error = sprintf('The following modules and/or themes are located inside the Sous profile directory. They must be moved elsewhere before Sous can be uninstalled: %s', implode(', ', $extensions));
      ...
      if ($children) {
        $warning = sprintf('The following install profiles use %s as a base profile. They must stand alone, or use a different base profile, before Sous can be uninstalled: %s', $info['name'], implode(', ', $children));
  ...
  /**
   * Performs required actions before Sous is uninstalled.
   *
   * @hook pre-command pm:uninstall
   */
  public function preCommand(): void {
    if ($this->getUninstall()) {
      if ($this->installProfile === 'sous') {
  ...
  /**
   * Performs required actions after Sous is uninstalled.
   *
  ...
  /**
   * Returns installed extensions in the Sous profile directory.
   *
   * @return string[]
   *   The names of installed extensions in the Sous profile directory.
   *   If there are any, Sous cannot be uninstalled. There shouldn't
   *   normally be any -- Sous doesn't ship with any runtime modules --
  ...
  private function getExtensionsInProfileDirectory(): array {
    ...
    $profile_path = $extensions['sous']->getPath();
    unset($extensions['sous']);
  ...
  /**
   * Alters the project-level composer.json to uninstall Sous.
   */
  private function alterProject(): void {
    ...
    $this->io()->listing([
      'Ensuring direct Sous dependencies are required',
    ...
    // Read Sous's composer.json, since we will need to merge in many
    ...
    assert($source['name'] === 'fourkitchens/sous-drupal-distro');
  ...
  private function getRequirements(array $target, array $source): array {
    $requirements = [];
    // Sous requires composer/composer in order for this uninstaller to
    ...
    // dependencies defined by the source package (Sous).
  ...
  private function getPathMap(array $target, array $source): array {
    ...
    // falling back to the source package (Sous) in the unlikely event
   ...
   *   The target package's configuration.
   * @param array $source
   *   The source package (i.e., Sous)'s configuration.
   *
   * @return string|null
